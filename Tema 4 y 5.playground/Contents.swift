//              --- Alumno: Edgar Leonardo Gutierrez Serna. ---
//              --- Matricula: 2809307. ---
...
var pastDate = Date(timeIntervalSinceNow: -60)
...
class User {
    var name:String
    var image:String
    var id = UUID().uuidString //Generacion random de ID
...
var user = User(name: "Leo", image: "Foto")
var post = Post(text: "Hola, Soy Leo, me gusta desarrollar aplicaciones en IOS", user: user)
var cantidadLike = 2


post.IncrementComment()//Se ingresa cuantos comentarios se hicieron.
post.IncrementLike()//Se ingresa cuantos likes se hicieron.

post.getUser().getID()
post.getUser().getName()

print("Usuario: \(user.getName()), ID: \(user.getID())")
